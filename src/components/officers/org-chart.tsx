          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-8 w-full max-w-4xl">
              ...
              <div className="flex-1 min-w-[280px] max-w-md">
                <div className="relative overflow-hidden rounded-2xl border border-amber-500/30 bg-card p-6 shadow-md backdrop-blur-md">
                  <div className="flex items-center gap-4">
                    <span className="grid size-12 place-items-center rounded-xl bg-amber-500/10 font-bold text-amber-600 text-sm">
                      {initials(adviser.name)}
                    </span>
                    <div>
                      <span className="inline-block rounded-full bg-amber-500/10 px-2.5 py-0.5 font-mono text-[10px] font-bold text-amber-700 uppercase tracking-wider">
                        FACULTY ADVISER
                      </span>
                      <h3 className="mt-1 text-lg font-extrabold tracking-tight">{adviser.name}</h3>
                      <p className="text-xs text-muted-foreground">IT Department Adviser</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {chairman && (
                <div className="flex-1 min-w-[280px] max-w-md">
                  <div className="relative overflow-hidden rounded-2xl border border-primary/40 bg-gradient-to-br from-primary/10 via-card/90 to-card p-6 shadow-md backdrop-blur-md">
                    <div className="flex items-center gap-4">
                      <span className="grid size-12 place-items-center rounded-xl bg-primary text-primary-foreground font-bold text-sm shadow-sm">
                        {initials(chairman.name)}
                      </span>
                      <div>
                        <span className="inline-block rounded-full bg-primary/15 px-2.5 py-0.5 font-mono text-[10px] font-bold text-primary uppercase tracking-wider">
                          EXECUTIVE CHAIRMAN
                        </span>
                        <h3 className="mt-1 text-lg font-extrabold tracking-tight">{chairman.name}</h3>
                        <p className="text-xs text-muted-foreground">Head of Association</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="w-full max-w-5xl rounded-2xl border border-border/80 bg-muted/20 p-6 backdrop-blur-md">
              <div className="mb-4 text-center font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                Directorate Office & Executive Secretariat
              </div>
              <div className="grid gap-4 sm:grid-cols-3">
                {viceChairInternal && (
                  <div className="rounded-xl border border-border/60 bg-card/70 p-4">
                    <span className="font-mono text-[10px] font-bold text-primary uppercase">INTERNAL AFFAIRS</span>
                    <h4 className="mt-1 text-sm font-bold">{viceChairInternal.name}</h4>
                    <p className="text-xs text-muted-foreground">{viceChairInternal.position}</p>
                  </div>
                )}
                {viceChairExternal && (
                  <div className="rounded-xl border border-border/60 bg-card/70 p-4">
                    <span className="font-mono text-[10px] font-bold text-primary uppercase">EXTERNAL AFFAIRS</span>
                    <h4 className="mt-1 text-sm font-bold">{viceChairExternal.name}</h4>
                    <p className="text-xs text-muted-foreground">{viceChairExternal.position}</p>
                  </div>
                )}
                {secretaryNode && (
                  <div className="rounded-xl border border-border/60 bg-card/70 p-4">
                    <span className="font-mono text-[10px] font-bold text-primary uppercase">SECRETARIAT</span>
                    <h4 className="mt-1 text-sm font-bold">{secretaryNode.name}</h4>
                    <p className="text-xs text-muted-foreground">{secretaryNode.position}</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="w-full">
              <div className="mb-6 text-center font-mono text-xs font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                {departments.map((dept) => {
                  ...
                  return (
                    <SpotlightCard key={dept.code} className="flex flex-col justify-between p-6">
                      <div>
                        {/* Department Header */}
                        <div className="border-b border-border/60 pb-3">
                          ...
                          <h4 className="mt-1 text-base font-extrabold tracking-tight">{dept.vp.name}</h4>
                          <p className="text-xs font-medium text-muted-foreground">{dept.vp.position}</p>
                        </div>
                        ...
                        {dept.vp.children && dept.vp.children.length > 0 && (
                          <div className="mt-4 space-y-3">
                            ...
                            {dept.vp.children.map((lead) => (
                              <div key={lead.name} className="relative border-l-2 border-primary/30 pl-3">
                                <h5 className="text-xs font-bold">{lead.name}</h5>
                                <p className="text-[11px] text-muted-foreground">{lead.position}</p>

                                {/* Sub-leads if any */}
                                {lead.children?.map((sub) => (
                                  <div key={sub.name} className="mt-1.5 pl-2 border-l border-border text-[10px]">
